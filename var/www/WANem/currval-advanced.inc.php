...
function get_current_values($interface, $del, $delJitter, $delCorrelation, $delDistribution, $loss, $lossCorrelation, $dup, $dupCorrelation, $reorder, $reorderCorrelation, $gap, $bandwidth, $corrupt, $sym, $disc, $limit, $msFound, $src, $srcSub, $dest, $destSub, $port, $addInt, $deleteInt) {
	...
	while (strpos($output, 'flowid', $i)!==false):
		...
		$secondMatch=false;
		//If there is another instance of match
		if (strpos($output, 'match', $i)) {
			if (strpos($output, 'flowid', $i)) {
				if (strpos($output, 'match', $i) < strpos($output, 'flowid', $i)) {
					//match is the second match for the same flowid
					$secondMatch=true;
				}
			} else {
				//match is the second match for the same flowid
		...
		if ($secondMatch==true) {
			...
			if ($destFound[$x]==false) {
				$dest[$x]="any";
				$destSub[$x]="";
			}
 		}

		//Check for a third occurance of the word 'match'
		//Get the part of $output after the second occurance of 'match'
		//$output=substr($output,$n);
		$thirdMatch=false;
		//If there is another instance of match
		if (strpos($output, 'match', $i)) {
